...
def main():
    ...
    df.to_csv('results/compare_20clients.csv', index=False)
    ...
    print(f"{'='*70}")
    print(f"Retrain:")
    print(f"  遗忘率: {results['Retrain_Forgetting']:.1f}%")
    print(f"  保持率: {results['Retrain_Retention']:.1f}%")
    print(f"  ASR: {results['Retrain_ASR']:.1f}%")
    print(f"  时间: {results['Retrain_Time']:.0f}s")

    print(f"\nFedForget:")
    print(f"  遗忘率: {results['FedForget_Forgetting']:.1f}%")
    print(f"  保持率: {results['FedForget_Retention']:.1f}%")
    print(f"  ASR: {results['FedForget_ASR']:.1f}%")
    print(f"  时间: {results['FedForget_Time']:.0f}s")
    print(f"  加速比: {results['Retrain_Time']/results['FedForget_Time']:.2f}×")

    print(f"\n✅ 结果已保存: results/compare_20clients.csv")
